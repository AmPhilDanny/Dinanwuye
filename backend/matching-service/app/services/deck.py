...
async def build_deck(
    ...
    for cand in candidates:
        if cand.userId in excluded or cand.userId in swiped:
            continue
    ...
    scored.sort(key=lambda t: (t[0], t[2].userId), reverse=True)

    if cursor:
        parts = cursor.split(":", 1)
        cursor_score = int(parts[0])
        cursor_id = parts[1] if len(parts) > 1 else ""
        scored = [t for t in scored if (t[0], t[2].userId) < (cursor_score, cursor_id)]

    page = scored[:limit]
    has_more = len(scored) > limit
    last = page[-1] if page else None
    next_cursor = f"{last[0]}:{last[2].userId}" if has_more and last else None
